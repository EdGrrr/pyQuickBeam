...
! function VALS_EQUAL
! ----------------------------------------------------------------------------  
  function vals_equal(val1,val2)
  implicit none
  
! Purpose:
!   Determine if a floating point value is close enough to an integer value
!     such that the two values can effectively be called equal
!
! Inputs:
!   [val1]   a floating point value
!   [val2]   an integer value
!
! Returns:
!   True if values are equal, false if they are not
!
! Created:
!   09/12/07  John Haynes (haynes@atmos.colostate.edu)
...
! ----- INTERNAL -----
  real*8, parameter :: small_number = 1E-5
    
  if (abs(val1-val2) < small_number) then
    vals_equal = .true.
  else
    vals_equal = .false.
  endif
...
! ----------------------------------------------------------------------------
! function VALS_DIFFER
! ----------------------------------------------------------------------------  
  function vals_differ(val1,val2)
  implicit none
  
! Purpose:
!   Determine if a floating point value is far enough from an integer value
!     such that the two values can definitely be called unequal
!
! Inputs:
!   [val1]   a floating point value
!   [val2]   an integer value
!
! Returns:
!   True if values are unequal, false if they are (at least nearly) equal
!
! Created:
!   09/12/07  John Haynes (haynes@atmos.colostate.edu)
