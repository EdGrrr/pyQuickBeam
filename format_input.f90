...
!   08/28/2006  John Haynes (haynes@atmos.colostate.edu)
! Modified:
!   09/13/2007  use binary sort for interpolation (JMH)
...
! ----- INTERNAL -----
  ...
  do i=1,nprof
    do k=1,ngate
      ind = -9
      t_matrix(i,k) =  linear_interpolate( &
                       env_hgt_matrix(i,:), &
                       env_t_matrix(i,:), &
                       hgt_matrix(i,k), &
                       ind &
                       )
      p_matrix(i,k) =  linear_interpolate( &
                       env_hgt_matrix(i,:), &
                       env_p_matrix(i,:), &
                       hgt_matrix(i,k), &
                       ind &
                       )
      rh_matrix(i,k) = linear_interpolate( &
                       env_hgt_matrix(i,:), &
                       env_rh_matrix(i,:), &
                       hgt_matrix(i,k), &
                       ind &
...
! ----------------------------------------------------------------------------
  subroutine order_data(hgt_matrix,hm_matrix,mf_matrix,p_matrix, &
...
!
! Outputs:
!   [hgt_matrix],[hm_matrix],[mf_matrix],[p_matrix,[t_matrix],[rh_matrix]
!   in proper order for input to the radar simulator routine
